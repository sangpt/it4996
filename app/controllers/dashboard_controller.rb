class DashboardController < ApplicationController
  def index
    ...
    @top_app = client.apps.inject({}){|hash, app| hash.merge(app.id.to_s => [app.name, app.requests.between_date(@start_date, @end_date).count])}
  ...
  def requests
    ...
    (client.requests.between_date(@start_date, @end_date) || client.requests.from_beginning_of_month)
  end
